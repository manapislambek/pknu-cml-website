...
// helper for year (handles bad/missing dates)
...
// Fetches and displays the 3 most recent publications — three stacked lines
async function loadRecentPublications() {
  ...
  try {
    const query = `*[_type == "publication"]{
      title, authors, publicationDate, journalName, volume, issue, pages, link
    ...
    const html = publications.map(pub => {
      ...
      const pages = (pub.pages || '').replace(/--/g, '–'); // 88--95 -> 88–95
      ...
      return `
        <article class="home-pub-card fade-in-element">
          ...
          <div class="home-pub-authors">
            ${pub.authors || ''} ${year ? `<span class="home-pub-year">(${year})</span>` : ''}
          </div>

          <!-- Line 3: Journal · Volume(Issue) · Pages + CTA -->
